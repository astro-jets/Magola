...
import { newPurchase } from "@/app/actions/purchases";
import SucessModal from "./modals/SuccessModal";


const Paywall = ({ property, onClose }: { property: PropertyProps, onClose: () => void }) => {
    ...
    const subscribe = async () => {
        setIsLoading(true)
        const res = await newPurchase(property._id!, session?.user.id)
        if (res) {
            setSuccess(true)
            setIsLoading(false)
        }
                                    ...
                                    <div className="lg:max-w-7xl max-w-xl mx-auto">
                                        <div className="grid lg:grid-cols-3 gap-10">
                                                ...
                                                <form className="mt-16 min-w-[50rem]">
                                                    ...
                                                    <div className="grid gap-6 mt-8">
                                                        <input type="text" placeholder="Cardholder's Name"
                                                            className="px-4 py-3.5 bg-white text-black w-full text-sm border-b-2 focus:border-[#333] outline-none" />
                                                    ...
                                                    <div className="flex flex-wrap gap-4 mt-8">
                                                        <button onClick={onClose} type="button" className="min-w-[150px] px-6 py-3.5 bg-red-500 text-sm bg-primary-700 text-white rounded-md hover:bg-gray-200">Cancel</button>
                                                        {!isLoading ?
                                                            <button onClick={() => { subscribe() }} type="button" className="min-w-[150px] px-6 py-3.5 text-sm bg-primary text-white rounded-md hover:bg-[#111]">
                                                                Confirm payment K {parseInt(property.cost).toLocaleString()}
                                                            </button> :
                                            ...
                                            <div className="bg-gray-100 px-6 py-8 rounded-md">
                                                ...
                                                <ul className="text-white mt-10 space-y-6">
                                                    {
                                                        property.tags?.map(item => (
                                                            <li className="flex flex-wrap gap-4 text-base">{item}</li>
                                                        ))
                                                    }
                                                    <li className="flex flex-wrap gap-4 text-base font-bold border-t-2 pt-4">Total <span className="ml-auto">K  {parseInt(property.cost).toLocaleString()}</span></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
