...
const LeaseForm = ({ property, onClose }: { property: PropertyProps, onClose: () => void }) => {
    ...
    return (
        <>
                                    ...
                                    <div className="lg:max-w-7xl max-w-xl mx-auto">
                                        <div className="grid lg:grid-cols-3 gap-10">
                                                    ...
                                                    <div className="flex flex-wrap gap-4 mt-8">
                                                        ...
                                                        {!isLoading ?
                                                            <button onClick={() => { subscribe() }} type="button" className="min-w-[150px] px-6 py-3.5 text-sm bg-primary text-white rounded-md hover:bg-[#111]">
                                                                Confirm payment K {(parseInt(property.cost) * 0.1).toLocaleString()}
                                                            </button> :
                                            ...
                                            <div className="bg-gray-100 px-6 py-8 rounded-md">
                                                <h2 className="text-2xl font-extrabold text-white">Lease payment for {property.name}</h2>
                                                <ul className="text-white mt-10 space-y-6">
                                                    ...
                                                    <li className="flex flex-wrap gap-4 text-base font-bold border-t-2 pt-4">Total <span className="ml-auto">K   {(parseInt(property.cost) * 0.1).toLocaleString()}</span></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
            ...
            <SucessModal
                isOpen={success}
                message={`Successfully applied for a Lease for ${property.name}`}
                onClose={() => { handleClose() }}
                title="Successfull"
                url=""
            />
